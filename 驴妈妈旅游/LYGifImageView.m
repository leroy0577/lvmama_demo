...
{
    LYGifImageView * gifImageView = [[LYGifImageView alloc]initWithGIFData:gifImageData];
    if (!gifImageView)
    {
        return nil;
    }
...
//{
//    return [self initWithGifImage:image];
//}
...
{
    if (gifImageDta.length <4)
    {
        return nil;
    }
    if (![LYGifImageView isGifImage:gifImageDta])
    {
        UIImage * image = [UIImage imageWithData:gifImageDta];
        return [super initWithImage:image];
    }
    [self decodeGIF:gifImageDta];
